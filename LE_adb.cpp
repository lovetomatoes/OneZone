...
void profile_adb_Kc(char* filename, double R, double n_adb, double z=z1, double Mh=Mh1){
    ...
    file<<"xi r y0 y1 theta_n n_g n_DM T_g\n";
    // dx
    double dx;
    double err=.01;
    // boundary conditions
    ...
    for (i=1;i<N;i++){
        ...
        file<<" "<<y[i][0]<<" "<<y[i][1];
        file<<" "<<pow(y[i][0],n_adb)<<" "<<ng0*pow(y[i][0],n_adb);
        file<<" "<<halo1.rho_c/( alpha*x[i] * pow((1+alpha*x[i]),2) )/(mu*m_H);// n_DM
        file<<" "<<Kc* (mu*m_H) *pow( rho_g0*pow(y[i][0],n_adb) , gamma-1.) /k_B; //T_g
        file<<endl;
        }
    }
  ...
  // file<<"xi r y0 y1 n_g\n";
    ...
    for (i=1;i<N;i++){
      ...
      if (i%10 ==0){
        ...
        file<<" "<<ng0*y[i][0];
        file<<" "<<halo1.rho_c/( alpha*x[i] * pow((1+alpha*x[i]),2) )/(mu*m_H);// n_DM
        file<<" "<<Kcore*(1+a*x[i]/r_core)* (mu*m_H) *pow(rho_g0*y[i][0], gamma-1.) /k_B; // T_g
        file<<endl;
      }
    }
...
void Mg2N0_adb(double& n_sol, double ni, double z, double Mh){
  ...
  // get solution of given Mg=fb*Mh
    ...
    while ( pow(dR,2)>=epE2*pow(R0,2) ){ //dR < 0.1 R0 or it=6, 结束计算 && it<5
        ...
        // printf("R0-=%3.2e, Mg_bw=%3.2e, R0+=%3.2e, Mg_fw%3.2e\n",R0-0.5*delta_R, Mg_bw/Ms, R0+0.5*delta_R, Mg_fw/Ms);
        // printf("dMdR=%3.2e, delta_R=%3.2e, Mg_0=%3.2e\n",dMdR/Ms, delta_R, Mg_0/Ms);
        // printf("fbMh=%3.2e, dR=%3.2e\n", fb*Mh/Ms, dR);
        // printf("\nin Mg2N0_ADB LOOP: it=%d, n0=%3.2e, dR/R0 = %3.2e\n",it, R0*halo.rho_c/(mu*m_H), dR/R0);
    // update R0
        if (R1<0) R0 = R0/10.;
        else R0 = R1;
...
/* int main(){
  ...
  // 1. Tvir v.s. n_sol
    ...
    //     BOUNDARY_adb(n_vir, Mg_vir, r_out,Tg_ave, R, z, Mh);
    //     f<<Tv<<" "<<n_sol<<" "<<n_vir<<" "<<n_nfw<<" "<<Mh/Ms<<" "<<Mg_vir/Ms;
    //     f<<" "<<6* pow(halo1.Tvir/1000., 1.5)<<" "<<N_CORE(z)<<" "<<0.1*halo1.Kvir<<" "<<K_ISM(z)<<" "<<Tg_ave<<" "<<r_out<<endl;
    //     Tv *= Tvrat;
    // }
    //  f.close();
  // 2. z v.s. n_sol
    ...
    //     n_nfw = fb*halo1.Rho_r(halo1.Rvir)/(mu*m_H);
    //     BOUNDARY_adb(n_vir, Mg_vir, r_out,Tg_ave, R, z, Mh);
    //     f<<z<<" "<<n_sol<<" "<<n_vir<<" "<<n_nfw<<" "<<Mh/Ms<<" "<<Mg_vir/Ms;
    //     f<<" "<<6* pow(halo1.Tvir/1000., 1.5)<<" "<<N_CORE(z)<<" "<<0.1*halo1.Kvir<<" "<<K_ISM(z);
    //     f<<" "<<halo1.Tvir<<" "<<Tg_ave<<" "<<r_out<<endl;
    //     z *= zrat;
    // }
    // f.close();
