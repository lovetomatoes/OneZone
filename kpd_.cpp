...
//     H2+ photodetachment
   ...
   /* 读取没问题
   for(i=0; i<nT; i++){
      for(j=0; j<nnu; j++) cout<<sigmaa[i][j]<<"\t";
      cout<<endl<<endl<<endl;
   }*/
   printf("k_Hm=%3.2e, k_H2p=%3.2e, kHm/kH2=%3.2e\n",
   k_Hm*1.e-21/Flux[k_ly], k_H2p*1.e-21/Flux[k_ly] ,k_Hm*1.e-21/Flux[k_ly]/1.39e-12);
...
void bilinear(double* x1a, double* x2a, double** ya, int m, int n, double x1, double x2, double& y){
   ...
   if((x1<x1a[0]) or (x1>x1a[m-1]) or (x2<x2a[0]) or (x2>x2a[n-1])) cout<<"IN BILINEAR, WRONG BOUNDARY"<<endl;
      ...
      y3=ya[ms][ns];
      y4=ya[ms-1][ns];
      printf("IN BILINEAR:y1=%3.2e, y2=%3.2e, y3=%3.2e, y4=%3.2e\n",y1,y2,y3,y4);
